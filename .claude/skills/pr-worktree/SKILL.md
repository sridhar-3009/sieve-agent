...
description: >
  ...
  for sieve-agent, without leaking API keys or leaving branches behind. Use when
...
git -C $REPO worktree add -q ~/Developer/sieve_agent-prs/pr$N pr-$N
ln -sfn $REPO/.sieve ~/Developer/sieve_agent-prs/pr$N/.sieve   # real memory/traces/calendar
```
...
```bash
ln -sfn $REPO/.env ~/Developer/sieve_agent-prs/pr$N/.env
```
...
```bash
[ -L ~/Developer/sieve_agent-prs/pr$N/.env ] && echo "symlink (safe)" || echo "REAL FILE — copy of all keys"
```
...
```bash
cd ~/Developer/sieve_agent-prs/pr$N
SIEVE_DASHBOARD_PORT=7778 $REPO/.venv/bin/python -m sieve_agent.ops.dashboard
```
...
for d in ~/Developer/sieve_agent-prs/*/; do git -C $REPO worktree remove --force "$d"; done
git -C $REPO worktree prune
git -C $REPO branch --list 'pr-*' | tr -d ' ' | xargs -r -n1 git -C $REPO branch -D
# 4. verify nothing is left holding keys
find ~/Developer/sieve_agent-prs -name ".env" 2>/dev/null
```
...
- [ ] `find ~/Developer/sieve_agent-prs -name ".env"` prints nothing
