...
real-Sieve factory for live ones."""
...
def _has_key() -> bool:
    """True when the ACTIVE provider (SIEVE_PROVIDER) has its key set, so live
...
def make_waku(home: Path, client=None, **settings_overrides):
    """Build a Sieve with an isolated home dir; optionally swap in a fake client."""
    from sieve_agent.app import Sieve
    from sieve_agent.config import Settings

    # A test must describe its own world. `sieve_agent/config.py` calls load_dotenv()
    ...
    #                          a stale SIEVE_GRAPH_WORKFLOWS=1 ate a scripted
    ...
    return Sieve(settings=settings, client=client)
