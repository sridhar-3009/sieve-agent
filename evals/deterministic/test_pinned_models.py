...
The shortlist lives in .sieve/models.json as {"pinned": ["provider:model", ...]};
...
from sieve_agent.ops import settings_api as d
...
def home(tmp_path, monkeypatch):
    ...
    # so SIEVE_PROVIDER must also be tracked to prevent leaking into later
    # tests (test_tool_trigger would inherit a stale provider and crash).
    monkeypatch.delenv("SIEVE_PROVIDER", raising=False)
    return tmp_path
