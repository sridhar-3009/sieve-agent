...
it keeps a single Sieve behind `sieve_agent.ops.browser_agent`. Two callers mutate it:
...
fresh Sieve starts on the eternal 'default' session. So one visit to the Settings
...
from sieve_agent.ops import browser_agent
...
def test_a_failed_rebuild_keeps_the_working_agent(isolated, monkeypatch):
    ...
    monkeypatch.setattr("sieve_agent.app.Sieve", explode)
