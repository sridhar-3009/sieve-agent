...
sieve_agent runs on people's own machines with their own keys, and it writes a lot of
...
The gap this was written for (2026-07-26): .gitignore listed `.sieve/` by exact
...
second home was called `.sieve-discord/`, matched nothing, and its SOUL.md,
usage.jsonl and outbox/ were tracked. A privacy fix had quietly created a
different privacy hole. `.sieve-*/` closes it for any alternate home.
...
# alternate one (DISCORD_HOME, or any SIEVE_HOME a user points elsewhere).
ARTIFACTS = ["state.db", "SOUL.md", "usage.jsonl", "calendar.ics",
             "traces/2026-07-26.jsonl", "outbox/msg-1.txt"]


@pytest.mark.parametrize("home", [".sieve", ".sieve-discord", ".sieve-demo"])
...
def test_the_env_template_carries_no_real_values():
    ...
    # SIEVE_PROVIDER is a non-secret default and is meant to have a value.
    unexpected = [ln for ln in filled if not ln.startswith("SIEVE_PROVIDER=")]
