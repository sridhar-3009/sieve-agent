...
from sieve_agent.loop.models import PROVIDERS, OpenAICompatClient, get_client
...
def test_dashboard_pricing_covers_every_provider(name):
    from sieve_agent.ops.pricing import PRICING
...
def test_model_listing_falls_back_without_a_catalog(name, monkeypatch):
    ...
    from sieve_agent.ops import catalog

    monkeypatch.setenv("SIEVE_PROVIDER", name)
    monkeypatch.delenv("SIEVE_MODEL", raising=False)
    monkeypatch.delenv("SIEVE_SMALL_MODEL", raising=False)
...
def test_price_for_layers_model_over_provider():
    ...
    from sieve_agent.ops.pricing import MODEL_PRICING, PRICING, price_for
...
def test_every_priced_model_has_a_knowledge_cutoff():
    ...
    from sieve_agent.ops.pricing import MODEL_CUTOFF, MODEL_PRICING, cutoff_for
