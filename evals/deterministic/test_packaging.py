...
1. THE WHEEL SHIPPED NO SKILLS. `pyproject.toml` packages `sieve_agent`, but the
   bundled skills live in `skills/` at the REPO ROOT, so `pip install
   sieve-agent` produced a Sieve with zero procedural memory — one of the four
   pillars, silently absent. The dashboard shipped fine (it lives under
   `sieve_agent/ops/static`), which is exactly why nobody noticed: the thing you can
...
2. THE TEST SUITE READ THE DEVELOPER'S `.env`. `sieve_agent/config.py` calls
   load_dotenv() at import, so a stale `SIEVE_GRAPH_WORKFLOWS=1` routed every
...
def test_the_bundled_skills_are_findable():
    """Whatever the install shape, Sieve must find the skills it ships with."""
    dirs = bundled_skill_dirs()
    assert dirs, (
        "bundled_skill_dirs() found nothing — Sieve would start with no "
...
def test_the_wheel_carries_the_skills_folder():
    """The build config half of the fix. `packages = ["sieve_agent"]` alone leaves the
    repo-root skills/ out of the wheel, which is the bug — force-include copies
    it to sieve_agent/skills so an installed Sieve finds it beside the code."""
    wheel = _pyproject()["tool"]["hatch"]["build"]["targets"]["wheel"]
    assert wheel.get("force-include", {}).get("skills") == "sieve_agent/skills", (
        "pyproject no longer force-includes skills/ into the wheel — a "
        "`pip install sieve-agent` would ship with zero skills again"
    )


def test_lookup_covers_both_install_shapes():
    """The lookup half. A checkout has skills/ at the repo root; a wheel has it
    at sieve_agent/skills. Exactly one exists at a time, so the function must look in
    both — checking only one is how this broke."""
    import inspect

    from sieve_agent import memory
...
def test_a_scripted_turn_ignores_the_graph_flag(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SIEVE_GRAPH_WORKFLOWS", "1")
