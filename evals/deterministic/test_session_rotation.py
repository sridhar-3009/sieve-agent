...
is older than SIEVE_SESSION_IDLE_MINUTES, the next chat starts a new thread
(old one stays in History)."""

from __future__ import annotations

from evals.helpers import ScriptedClient, make_waku
from sieve_agent.ops.browser_agent import maybe_rotate_session
...
def test_idle_session_rotates(tmp_path, monkeypatch):
    monkeypatch.setenv("SIEVE_SESSION_IDLE_MINUTES", "60")
...
def test_rotation_can_be_disabled(tmp_path, monkeypatch):
    monkeypatch.setenv("SIEVE_SESSION_IDLE_MINUTES", "0")
    app = make_waku(tmp_path / "home", client=ScriptedClient([]))
    before = app.session.session_id
...
def test_provider_switch_resets_stale_model_overrides(tmp_path, monkeypatch):
    ...
    didn't newly type."""
    from sieve_agent.ops import settings_api

    captured = {}
    monkeypatch.setenv("SIEVE_PROVIDER", "kimi")
    monkeypatch.setenv("SIEVE_MODEL", "kimi-k3")
    monkeypatch.setenv("SIEVE_SMALL_MODEL", "kimi-k3")
    ...
    try:
        ...
        pass
    assert captured.get("SIEVE_MODEL", "unset") in ("", "unset") or \
        captured.get("SIEVE_PROVIDER") == "gemini"
    # the actual contract: stale kimi ids must have been blanked
    assert captured.get("SIEVE_MODEL") != "kimi-k3"
    assert captured.get("SIEVE_SMALL_MODEL") != "kimi-k3"
