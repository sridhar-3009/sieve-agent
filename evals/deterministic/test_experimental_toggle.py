...
from `load_settings()`, which reads SIEVE_EXPERIMENTAL from the environment.
...
from sieve_agent.config import Settings, load_settings
from sieve_agent.tools import build_registry


def test_settings_exposes_the_flag_so_the_ui_can_render_a_toggle(monkeypatch):
    from sieve_agent.ops import settings_api

    monkeypatch.delenv("SIEVE_EXPERIMENTAL", raising=False)
    assert settings_api.settings_info()["experimental"] is False

    monkeypatch.setenv("SIEVE_EXPERIMENTAL", "1")
...
def test_an_explicit_setting_beats_the_global_env_switch(tmp_path, monkeypatch):
    ...
    SIEVE_EXPERIMENTAL, then switching chat delegation on (which writes that var)
    ...
    monkeypatch.setenv("SIEVE_EXPERIMENTAL", "1")      # global switch ON
...
def test_env_var_is_what_load_settings_reads(monkeypatch):
    """Why the chat was broken: nothing sets SIEVE_EXPERIMENTAL for the server."""
    monkeypatch.delenv("SIEVE_EXPERIMENTAL", raising=False)
    assert load_settings().experimental is False
    monkeypatch.setenv("SIEVE_EXPERIMENTAL", "1")
    assert load_settings().experimental is True
    assert os.getenv("SIEVE_EXPERIMENTAL") == "1"
