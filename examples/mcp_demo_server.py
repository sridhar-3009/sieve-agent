"""A tiny, self-contained MCP server — the demo connector for sieve-agent.
...
so the connector story runs with zero extra installs:
    ...
    pip install -e '.[mcp]'
    cp examples/mcp.demo.json .sieve/mcp.json
...
that's the whole point: connectors plug in without changing Sieve's code.
"""
...
if __name__ == "__main__":
    mcp.run()  # stdio transport — how Sieve's MCPBridge talks to it
