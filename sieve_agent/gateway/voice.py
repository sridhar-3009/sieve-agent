...
only moves words in and out (that's the whole point of the gateway box).
  ...
  voice  macOS `say` with a British voice by default (zero setup), or the
         ...
         then set SIEVE_TTS=kokoro  (SIEVE_VOICE=bm_george / bm_fable / ...)
...
from sieve_agent.gateway.cli import _observer  # show gate/tool lines in voice mode too
...
class Ears:
    def __init__(self, model_size: str | None = None):
        ...
        self.model = WhisperModel(
            model_size or os.getenv("SIEVE_WHISPER_MODEL", "base"),
            compute_type="int8",
        )

    def transcribe(self, audio, language: str | None = None) -> str:
        segments, _ = self.model.transcribe(
            audio, language=language or os.getenv("SIEVE_WHISPER_LANG")
        )
...
class Mouth:
    ...
    def __init__(self):
        self.engine = os.getenv("SIEVE_TTS", "").strip().lower()
        self.voice = os.getenv("SIEVE_VOICE", "")
        if not self.engine:
    ...
    def speak(self, text: str) -> None:
        ...
        if self.engine == "kokoro":
            ...
            print("(no TTS engine on this platform — set SIEVE_TTS=kokoro)")


def matches_wake(text: str, wake_word: str) -> bool:
    """Does a transcript contain the (customizable) wake word?

    Fuzzy on purpose: Whisper mis-hears short wake phrases constantly ("hey
    sieve" as "hey siv", "heysieve", or with the trailing consonant dropped
    entirely). So: `wake_word` accepts comma-separated variants across
    scripts, normalization keeps kana AND CJK for non-latin phrases, and
    matching is substring + sliding-window similarity. Pure function →
    deterministic evals.
    """
    import difflib
    import re
...
def _mic_threshold() -> float:
    ...
    SIEVE_MIC_THRESHOLD (lower if it never hears you, higher if it
    wakes on room noise)."""
    return float(os.getenv("SIEVE_MIC_THRESHOLD", "0.005"))
...
def wake_loop(sieve_agent: Sieve, mouth: Mouth, wake_word: str) -> None:
    ...
    - The mic buffer is drained after Sieve speaks, so it doesn't wake on
    ...
    ack = os.getenv("SIEVE_WAKE_ACK", "Yes?")
    followup = float(os.getenv("SIEVE_FOLLOWUP_SECONDS", "8"))  # stay open, Siri-style
    block = SAMPLE_RATE // 10
    # Pin the scout's transcription language to match the wake word's script —
    # otherwise Whisper can transcribe a latin phrase into another script
    # entirely, which a latin wake word never matches. Commands after wake
    # still auto-detect.
    wake_lang = os.getenv("SIEVE_WAKE_LANG") or ("en" if wake_word.isascii() else None)
        ...
        while True:
            ...
            if not matches_wake(heard_scan, wake_word):
                ...
                if heard_scan:  # near-misses belong in the trace (wake tuning!)
                    sieve_agent.tracer.event("wake_scan", {"heard": heard_scan, "matched": False})
                continue
            ...
            # for a follow-up for `followup` seconds — no need to repeat the
            # wake word (like Siri). A quiet stretch drops back to wake-word mode.
            while True:
                heard = ears.transcribe(record_command(stream))
                if heard:
                    print(f"you › {heard}")
                    result = sieve_agent.respond(heard, observer=_observer, source="voice")
...
def main() -> None:
    ...
    sieve_agent = Sieve()
    sieve_agent.session.session_id = "voice"   # its own conversation thread in the inbox
    mouth = Mouth()

    # Hands-free by default: always-listening for "hey sieve". The default packs
    # in the ways the tiny scanner mis-hears it, so it triggers reliably.
    # Set SIEVE_WAKE_WORD="" for push-to-talk instead.
    wake_word = os.getenv(
        "SIEVE_WAKE_WORD", "hey sieve,heysieve,hey siv,hey seave,hey seeve"
    ).strip()
    if wake_word:
        try:
            wake_loop(sieve_agent, mouth, wake_word)
    ...
    ears = Ears()
    print("Voice Sieve ready. Press Enter to talk, Ctrl-C to quit.")
    while True:
        try:
            input("\npress Enter to talk… ")
