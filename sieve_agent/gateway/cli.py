"""CLI gateway — the zero-setup way to talk to your Sieve.
...
def _memory_snapshot(conn: sqlite3.Connection) -> str:
    """Render a bounded, read-only view of Sieve's local memory."""
...
def main() -> None:
    sieve_agent = Sieve()
    sieve_agent.session.session_id = "terminal"   # its own conversation thread in the inbox
    console.print(Panel.fit(
        "[bold]Sieve[/bold] — local, yours, transparent.\n"
        f"home: {sieve_agent.settings.home.resolve()}   model: {sieve_agent.settings.model}\n"
    ...
    while True:
        ...
        if user_message == "/memory":
            console.print(
                Panel(
                    Text(_memory_snapshot(sieve_agent.conn)),
                    title="Memory snapshot",
                    border_style="cyan",
                )
            )
            continue
        result = sieve_agent.respond(user_message, observer=_observer, source="cli")
        console.print(f"[bold green]sieve_agent ›[/bold green] {result.reply}\n")
