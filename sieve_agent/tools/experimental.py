...
contractor (read/bash/edit/write, pure coding craft). Sieve hires; pi codes;
Sieve's release gate can then inspect the work.
...
`SIEVE_EXPERIMENTAL=1` to register these tools.
"""
...
def _project_pi_flags() -> list[str]:
    ...
    This is the "pi x sieve_agent" payoff: sieve_agent upgrades its coding contractor without
...
def make_delegate_tool(settings: Settings) -> Tool:
    ...
    Same honesty contract as every Sieve tool — the return string says exactly
    ...
    def delegate_task(task: str = "", cwd: str = "", timeout_seconds: int = 0,
        ...
        from sieve_agent.tools import workspace
        if cwd:
        ...
        timeout = int(timeout_seconds) or int(os.getenv("SIEVE_DELEGATE_TIMEOUT", "300"))
        ...
        from sieve_agent.ops.coding_eval import PI_PROVIDER, _key_for
        cmd = [pi_bin]
        ...
        if json_mode:
            ...
            if code is None:
                return (f"pi was still working after {timeout}s so I stopped it — try a smaller "
                        f"task, or raise SIEVE_DELEGATE_TIMEOUT.")
            stdout_text = reply
        else:
            try:
...
def make_tools(settings: Settings) -> list[Tool]:
    """Experimental tools, registered only when SIEVE_EXPERIMENTAL=1: the live
