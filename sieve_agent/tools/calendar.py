...
Where events land:
  ...
  opt-in      Apple Calendar, in a dedicated "Sieve" calendar, via AppleScript —
              set SIEVE_APPLE_CALENDAR=1. First use makes macOS ask permission
              for your terminal to control Calendar; approve once.
  opt-in      Google Calendar via SIEVE_GOOGLE_CALENDAR=1. Local files remain
...
relays it, so Sieve never over-claims what happened.
"""
...
def sync_to_apple_calendar(title: str, start: str, end: str, notes: str = "") -> str:
    """Create the event in Calendar.app under the 'Sieve' calendar (created on
    ...
    # Prefer a dedicated "Sieve" calendar, but macOS can't create calendars in
...
def make_tool(
    ...
    def create_event(
        ...
        if not apple_calendar and not google_calendar:
            where += (
                " Not synced to any calendar app (enable with SIEVE_APPLE_CALENDAR=1 "
                "or SIEVE_GOOGLE_CALENDAR=1, "
...
def make_list_tool(conn: sqlite3.Connection, home: Path | None = None) -> Tool:
    ...
    sieve_agent itself created. Every source is LABELLED in the output, so the agent can
    ...
    def list_events(start: str = "", end: str = "", limit: int = 20) -> str:
        ...
        if home is not None:
            from sieve_agent.tools.google_calendar import is_connected, list_google_events
        ...
        checked.append("sieve_agent's local calendar")
        local = local_events(start, end, limit)
        if local:
            sections.append("From sieve_agent's local calendar (events sieve_agent created):\n" + local)
    ...
    return Tool(
        ...
        description=(
            ...
            "when signed in, plus sieve_agent's own local calendar). "
