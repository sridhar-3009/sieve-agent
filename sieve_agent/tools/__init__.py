...
Apple ecosystem (SIEVE_APPLE_TOOLS=1) and MCP servers (.sieve/mcp.json)."""
...
from sieve_agent.tools.registry import ToolRegistry
...
def build_registry(conn: sqlite3.Connection, settings: Settings, memory=None) -> ToolRegistry:
    ...
    # source (Google when signed in, plus sieve_agent's own), so the model never has
    ...
    # Experimental tools — off by default; opt in with SIEVE_EXPERIMENTAL=1.
    ...
    # SIEVE_EXPERIMENTAL, so re-checking the env here would let the global switch
    # override an explicit False — and the arena passes experimental=False for
    # every non-coding race. Once the dashboard could write SIEVE_EXPERIMENTAL=1,
    # that OR silently forced delegate_task into races that never asked for it.
    if getattr(settings, "experimental", False):
        from sieve_agent.tools import experimental
    ...
    # MCP servers (opt-in via .sieve/mcp.json).
    mcp_config = settings.home / "mcp.json"
    if mcp_config.exists():
        try:
            from sieve_agent.tools.mcp_client import MCPBridge

            bridge = MCPBridge(mcp_config)
            for t in bridge.start():
                registry.register(t)
            registry.mcp_bridge = bridge  # so Sieve.close() can stop the servers
        except ImportError:
            print("mcp.json found but the 'mcp' package is missing — pip install 'sieve-agent[mcp]'")
