...
-- confidence/status/superseded_by/previous_id/last_accessed_at/access_count
-- back the write gate, contradiction resolution, and decay scoring in
-- sieve_agent/memory/semantic/ — see write_gate.py, contradiction.py, decay.py.
CREATE TABLE IF NOT EXISTS facts (
    ...
    confidence REAL DEFAULT 1.0,       -- write-gate score at insert time
    status TEXT DEFAULT 'active',      -- active | superseded | archived
    superseded_by INTEGER REFERENCES facts(id),
    previous_id INTEGER REFERENCES facts(id),   -- prior version in this subject's timeline
...
def _migrate(conn: sqlite3.Connection) -> None:
    ...
    for name, ddl in (
        ("confidence", "ALTER TABLE facts ADD COLUMN confidence REAL DEFAULT 1.0"),
        ...
        ("previous_id", "ALTER TABLE facts ADD COLUMN previous_id INTEGER"),
        ("last_accessed_at", "ALTER TABLE facts ADD COLUMN last_accessed_at TEXT DEFAULT (datetime('now'))"),
