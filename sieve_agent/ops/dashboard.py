...
One stdlib HTTP server reading the files Sieve already writes:
...
from sieve_agent.ops import browser_agent, commands, compare_history
from sieve_agent.ops.arena import (
    compare_clear,
    compare_delete_run,
    compare_regrade,
    compare_stream,
...
from sieve_agent.ops.browser_agent import agent_lock, dash_session, get_agent, maybe_rotate_session
from sieve_agent.ops.catalog import list_models
from sieve_agent.ops.pricing import price_for, usage_summary
from sieve_agent.ops.settings_api import apply_settings, pin_action, settings_info
from sieve_agent.ops.tracing import TraceEncodingError, iter_trace_lines
...
def _get_notion_store():
    ...
    if _notion_store is None:
        from sieve_agent.memory.episodic.notion_store import NotionEpisodeStore
...
def collect() -> dict:
    ...
    from sieve_agent.memory import bundled_skill_dirs
    from sieve_agent.memory.procedural.loader import SkillLoader
    ...
    from sieve_agent.graph.workflows.gather import gather_topology
    from sieve_agent.graph.workflows.triage import triage_topology
    ...
    return {
        ...
        "facts": rows("SELECT id, subject, content, source, status, confidence, superseded_by, created_at "
                      "FROM facts ORDER BY (status='active') DESC, id DESC"),
        ...
        "graph": {
            ...
            # `sieve_agent gather` is a routine you run yourself and ignores this flag
...
def _rel_to_home(path, home) -> str:
    """Path relative to SIEVE_HOME if it lives there, else the repo-relative
...
def session_list(conn) -> list[dict]:
    ...
    for g in groups:
        ...
        preview = ""
        if last:
            preview = ("you: " if last["role"] == "user" else "sieve_agent: ") + last["content"][:80]
        out.append({"id": sid,
...
def tools_info() -> dict:
    ...
    if live is not None:
        ...
        from sieve_agent.tools import calendar, memory_admin, messages, notes, search
        ...
        tools = [calendar.make_tool(
                 ...
                 notes.make_tool(mem), messages.make_tool(settings.home),
                 search.make_tool(),
        ...
        if settings.experimental:
            ...
            # switch did nothing.
            from sieve_agent.tools import experimental as experimental_tools
...
def transcribe_audio(raw: bytes) -> dict:
    ...
    with _whisper_lock:
        if _whisper is None:
            _whisper = WhisperModel(os.getenv("SIEVE_WHISPER_MODEL", "base"), compute_type="int8")
...
def _editor_cmd() -> list[str] | None:
    """The user's code editor CLI: $SIEVE_EDITOR, then cursor, then code."""

    custom = os.getenv("SIEVE_EDITOR")
...
def reveal_path(rel: str) -> dict:
    """Open a file/folder under SIEVE_HOME — in the user's code editor if one
    is on PATH (cursor/code/$SIEVE_EDITOR), otherwise reveal in Finder.
    Restricted to paths inside SIEVE_HOME."""
    import subprocess
    import sys
    ...
    if target != home and home not in target.parents:
        return {"error": "path is outside the .sieve home"}
...
def memory_action(payload: dict) -> dict:
    ...
    from sieve_agent.memory.episodic.store import SqliteEpisodeStore
    ...
    if action == "save_skill":
        ...
        from sieve_agent.memory import bundled_skill_dirs
        from sieve_agent.memory.procedural.loader import _parse_text
...
def main() -> None:
    # Port precedence: SIEVE_DASHBOARD_PORT, then the conventional PORT (used by
    # deploy platforms and IDE preview panes), then 7777. If it's taken, walk on.
    base = int(os.getenv("SIEVE_DASHBOARD_PORT") or os.getenv("PORT") or PORT)
    for port in range(base, base + 10):  # walk past a busy port instead of crashing
        ...
        # too (background thread) so you don't need a separate `sieve_agent telegram`.
        try:
            from sieve_agent.gateway.telegram import start_in_background
        ...
        try:
            from sieve_agent.gateway.discord import start_in_background as start_discord
        ...
        try:
            from sieve_agent.gateway.whatsapp import start_in_background as wa_background

            if wa_background():
                print("WhatsApp gateway → listening in the background (webhook on port 5000)")
        except Exception as exc:  # noqa: BLE001 — never let a gateway block the dashboard
            print(f"(whatsapp) not started: {exc}")
        print(f"Sieve dashboard → http://localhost:{port}  (Ctrl-C to stop)")
