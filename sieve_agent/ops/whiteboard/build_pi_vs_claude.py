...
aligned rows each, then the sieve_agent punchline. Facts are sourced on the board
...
Run:  python -m sieve_agent.ops.whiteboard.build_pi_vs_claude
"""
...
ROWS = [
    ...
    ("SUB-AGENTS",
     ...
     "refused — spawn pi yourself: tmux,\nor sieve_agent's delegate_task (that's us)",
     "plain"),
    ("MCP",
...
def build() -> list:
    ...
    e += S.labeled_box(80, band, 2410, 96,
                       "sieve_agent uses BOTH: Claude Code writes sieve_agent's code - pi is sieve_agent's delegate_task sub-agent,\n"
