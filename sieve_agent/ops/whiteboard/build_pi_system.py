...
One tight landscape teaching board (companion to docs/whiteboards/pi-chart-prompt.md):
  ...
  FOOTERS     refused-on-purpose (red)  ·  sieve_agent x pi (blue)
...
Run:  python -m sieve_agent.ops.whiteboard.build_pi_system
"""
...
def build():
    ...
    # 3 container boundaries (loop / refused / sieve_agent), the pink model, the green
    ...
    a(S.text(78, 408, "↑ sieve_agent plugs in here", size=S.FS_SMALL, color=PAL["green"][1]))
    ...
    a(S.boundary(660, 925, 900, 205, "sieve_agent × pi", color="blue"))
    a(S.text(688, 985,
             "sieve_agent → delegate_task → pi -p --mode json\n"
             "events → arena card\n"
             "tokens → usage ledger",
             size=S.FS_BODY, color=INK))
