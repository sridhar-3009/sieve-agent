...
Plain static files served as-is by `sieve_agent/ops/dashboard.py` (a stdlib HTTP
...
- **The graph chart is data-driven — never hand-edit a topology.** `graphSVG`
  ...
  chart's shape, change the workflow in `sieve_agent/graph/workflows/`. Graph ids are
