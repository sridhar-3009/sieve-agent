// sieve_agent dashboard — settings write (applyModel), model picker/catalog/pins.
...
// Sieve's loop needs tool calling, so tool-less models are labelled as such.
...
async function loadModelList(){
  ...
  if (modelCatalog.listed){
    ...
    msg.textContent = `${ms.length} models on ${modelCatalog.endpoint}` +
      (free.length ? ` · ${free.length} free, ${freeTools.length} of those tool-capable (Sieve needs tool calling)` : "") +
