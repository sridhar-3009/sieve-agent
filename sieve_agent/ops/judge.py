...
Completion (sieve_agent.ops.scoring) is deterministic — did the right tool fire. Quality
...
pure text, no tools). Switchable per-race from the arena, or via SIEVE_JUDGE_*.
Any provider works — Sieve's OpenAI-compat client exposes the same
...
from sieve_agent.loop.models import get_client

JUDGE_PROVIDER = os.getenv("SIEVE_JUDGE_PROVIDER", "openai")
JUDGE_MODEL = os.getenv("SIEVE_JUDGE_MODEL", "gpt-5.6-sol")
...
_JUDGE_SEM = threading.Semaphore(int(os.getenv("SIEVE_JUDGE_CONCURRENCY", "2")))
