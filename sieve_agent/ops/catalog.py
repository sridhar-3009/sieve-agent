...
2. **What you chose.** `.sieve/models.json` holds an ordered `provider:model`
...
from sieve_agent.ops.pricing import remember_price
...
def list_models(provider: str | None = None) -> dict:
    ...
    OpenAI-compatible endpoints (OpenRouter, Gemini, any SIEVE_BASE_URL), or the
    ...
    # SIEVE_BASE_URL only applies to the provider it was set for).
    ...
    req = urllib.request.Request(url, headers={
        ...
        "User-Agent": "Mozilla/5.0 (compatible; Sieve)",
    })
    try:
...
def pinned_specs() -> list[str]:
    ...
    .sieve/models.json. The chat switcher shows exactly these. Before they've
