# sieve_agent/ops — the Eval/LLM-Ops pillar
...
whole directory and sieve_agent still runs. That's deliberate: ops observes, it never
participates.
...
| `browser_agent.py` | The ONE shared `Sieve` behind the browser gateway + its dated chat session. |
...
```python
from sieve_agent.ops.browser_agent import _agent   # frozen at None forever
```
