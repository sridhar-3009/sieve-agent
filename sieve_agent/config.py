...
file, you know everything Sieve can be configured to do.
"""
...
@dataclass
class Settings:
    # --- LLM: pick a provider, set its key. See sieve_agent/loop/models.py PROVIDERS.
    provider: str = field(default_factory=lambda: os.getenv("SIEVE_PROVIDER", "anthropic"))
    ...
    api_key: str = field(default_factory=lambda: os.getenv("SIEVE_API_KEY", ""))
    base_url: str | None = field(default_factory=lambda: os.getenv("SIEVE_BASE_URL") or None)
    model: str = field(default_factory=lambda: os.getenv("SIEVE_MODEL", ""))
    # Cheap model used by the retrieval gate and the consolidation summarizer.
    small_model: str = field(default_factory=lambda: os.getenv("SIEVE_SMALL_MODEL", ""))

    # --- Home: where Sieve keeps its state (memory DB, calendar, outbox, traces).
    # Defaults to ./.sieve next to where you run it, so you can open every file
    # it writes. Local-first means you can always look.
    home: Path = field(default_factory=lambda: Path(os.getenv("SIEVE_HOME", ".sieve")))

    # --- Loop guardrails
    max_iterations: int = field(default_factory=lambda: int(os.getenv("SIEVE_MAX_ITERATIONS", "10")))
    ...
    max_tokens: int = field(default_factory=lambda: int(os.getenv("SIEVE_MAX_TOKENS", "8192")))
    ...
    history_turns: int = field(default_factory=lambda: int(os.getenv("SIEVE_HISTORY_TURNS", "12")))
    ...
    # --- Tools
    # Sync created events into Apple Calendar (a dedicated "Sieve" calendar)
    # via AppleScript. Opt-in because it writes to your real calendar app.
    apple_calendar: bool = field(
        default_factory=lambda: os.getenv("SIEVE_APPLE_CALENDAR", "") in ("1", "true", "yes")
    )
    ...
    google_calendar: bool = field(
        default_factory=lambda: os.getenv("SIEVE_GOOGLE_CALENDAR", "") in ("1", "true", "yes")
    )
    google_calendar_id: str = field(
        default_factory=lambda: os.getenv("SIEVE_GOOGLE_CALENDAR_ID", "") or "primary"
    )
    ...
    apple_tools: bool = field(
        default_factory=lambda: os.getenv("SIEVE_APPLE_TOOLS", "") in ("1", "true", "yes")
    )
    ...
    # The gather workflow calls sieve_agent/tools/github.py as a library and does NOT
    # need this on — the switch only decides whether the MODEL can reach it.
    gh_tool: bool = field(
        default_factory=lambda: os.getenv("SIEVE_GH_TOOL", "") in ("1", "true", "yes")
    )
    # owner/name to assume when a call omits it — for when Sieve runs outside a
    # checkout, where `gh` has no remote to infer from.
    gh_repo: str = field(default_factory=lambda: os.getenv("SIEVE_GH_REPO", ""))
    ...
    experimental: bool = field(
        default_factory=lambda: os.getenv("SIEVE_EXPERIMENTAL", "") in ("1", "true", "yes")
    )
    ...
    # the plain loop, so this can never make Sieve worse — only faster/cheaper.
    graph_workflows: bool = field(
        default_factory=lambda: os.getenv("SIEVE_GRAPH_WORKFLOWS", "") in ("1", "true", "yes")
    )
