"""Wiring — builds one Sieve from its parts. Gateways call `respond()`.
...
from sieve_agent.loop.agent import LoopResult, Observer, run_loop
from sieve_agent.loop.models import get_client
from sieve_agent.ops.tracing import Tracer, compose
from sieve_agent.runtime.session import Session
...
class Sieve:
    ...
    def respond(self, user_message: str, observer: Observer | None = None,
        ...
        with self.tracer.turn(user_message):
            # The graph front door is optional and can NEVER make Sieve worse:
    ...
    def _respond_via_graph(self, user_message: str, notify, stream: bool) -> LoopResult | None:
        ...
        from sieve_agent.graph.workflows.triage import (
            QUICK_REPLY_PROMPT,
            build_triage_graph,
            classify_message,
            todays_events,
