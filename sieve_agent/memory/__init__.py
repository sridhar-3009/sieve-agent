...
from sieve_agent.memory import consolidation, retrieval_gate
...
def bundled_skill_dirs() -> list[Path]:
    """Where the skills that SHIP with Sieve live — and why there are two answers.
    ...
    the wheel only packages the `sieve_agent/` directory, so a `pip install sieve-agent`
    ...
    `sieve_agent/skills`, so an installed Sieve finds it next to the code.
...
class Memory:
    ...
    def export_markdown(self) -> None:
        ...
        lines = [
            "# Sieve memory",
            "",
            ("_A human-readable mirror of what Sieve remembers. The source of truth is "
