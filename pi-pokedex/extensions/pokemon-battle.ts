 ...
 *                     command that tries to wipe sieve_agent's runtime data (.sieve, the
...
// Team Rocket guard: hands off the Pokemon Center (.sieve runtime data).
// Blocks the exact mistake sieve_agent's own rules forbid — wiping .sieve or `rm -rf /`.
const ROCKET = /\brm\b[^\n]*\s(-[rRfF]+\s+)?(\.sieve(\/|\b)|\/\s|~\/?\s*$)/;
	...
	pi.on("tool_call", async (event) => {
		...
		if (ROCKET.test(command)) {
			return {
				block: true,
				reason:
					"Team Rocket guard: that command targets the Pokemon Center (.sieve " +
