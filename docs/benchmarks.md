# Model comparison — the Sieve benchmark battery

How Sieve compares models, what each test measures, and how to read the result.
...
Multi-step orchestration over Sieve's flagship tools (`create_event`,
...
Sieve is the orchestrator; **pi** is the coding contractor — but for a *coding*
...
**moonshotai/kimi**, xai/grok, zai/glm) — the runner maps Sieve's provider id to
...
[`sieve_agent/ops/coding_eval.py`](../sieve_agent/ops/coding_eval.py).
...
`SIEVE_WORKSPACE` (root), `SIEVE_DELEGATE_AUTORUN=0` (disable), `SIEVE_AUTORUN_TIMEOUT`.
See [`sieve_agent/tools/workspace.py`](../sieve_agent/tools/workspace.py).
...
Yes, Sieve already does Hermes / Claude-Code-style sub-agent spawning. It lives in
[`sieve_agent/tools/experimental.py`](../sieve_agent/tools/experimental.py) as `delegate_task`
and is **off by default** — set `SIEVE_EXPERIMENTAL=1` to register it.
...
- **What it is:** the "Sub-Agents" box on the architecture whiteboard, wired for
  ...
  (`pi -p "task"`).
- **The division of labor is the teaching point:** Sieve is the orchestrator
  (memory, working-memory assembly, the human's context, the release gate); pi
  is the specialist contractor (read / bash / edit / write). Sieve hires; pi
  codes; Sieve's gate inspects the work.
...
[`sieve_agent/ops/judge.py`](../sieve_agent/ops/judge.py). The referee is **switchable from the
...
(grading is pure text). Any provider works — Sieve's OpenAI-compat client gives
...
#   → also writes a timestamped .md + .json to .sieve/shootout/
...
**What Sieve already has:** the case format (`dataset.jsonl`), deterministic
...
- ~~Completion column wired into the live arena~~ — **done**: a race on a known
  ...
  [`sieve_agent/ops/scoring.py`](../sieve_agent/ops/scoring.py) shared with `shootout.py`.
...
- ~~Quality column (K3-as-judge) in the arena~~ — **done**: the "grade with K3"
  toggle judges each reply 0-10 ([`sieve_agent/ops/judge.py`](../sieve_agent/ops/judge.py));
...
markdown + json report to `.sieve/shootout/` anyone can reproduce with their keys.
