...
`sieve_agent/loop/agent.py` is one agent turn: a while-loop where the model picks tools until
...
Build a small engine in-repo. Reasons, in Sieve terms:
...
## 3. The engine — `sieve_agent/graph/`
...
## 4. Workflows in Sieve (use cases 2 & 3)
