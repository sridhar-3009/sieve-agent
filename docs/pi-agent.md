...
| `pi --mode json` | **event stream** on stdout, one JSON per line ← *sieve_agent plugs in here* |
...
```bash
claude -p "Reply with exactly: sieve_agent sieve_agent"   # answers, dies
claude    "Reply with exactly: sieve_agent sieve_agent"   # banner, state, waits for you
```
...
| sub-agents | spawn more pi's (tmux — or sieve_agent's `delegate_task`) |
...
## 4 · sieve_agent × pi — the collab thesis

```
sieve_agent loop → delegate_task → subprocess → pi -p --mode json
     pi's events → the arena card (live mini-terminal)
     pi's tokens → sieve_agent's usage ledger (coding runs aren't free)
```

Wiring: [`sieve_agent/tools/experimental.py`](../sieve_agent/tools/experimental.py) — every
...
| 4 | The League | **sieve_agent × pi** | `delegate_task` |
...
# 2b — guard a verb (throwaway dir with a dummy .sieve)
pi -e .pi/extensions/pokemon-battle.ts … -p "Run: rm -rf .sieve"
#  → "…blocked by a safety guard. The .sieve directory is protected…"
#  → .sieve SURVIVED. The refusal came back as a tool result and the model
...
### Stage 4 — sieve_agent × pi
```bash
SIEVE_EXPERIMENTAL=1 make run        # + make dashboard (:7777)
# ask sieve_agent: "delegate to pi: what beats Charizard? use the pokedex + type_matchup"
```
...
- **pi runs the model raw, and it can wander.** On one run, haiku ignored "what
  ...
  Why: pi loads `AGENTS.md`/`CLAUDE.md` from the repo, and sieve_agent's says *"commit
...
machine, pi 0.82.0, 2026-07-24/25*
