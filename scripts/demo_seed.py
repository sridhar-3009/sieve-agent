"""Reset .sieve to a clean, curated state for a demo / recording.
...
What it does (your old state is backed up first, never just deleted):
  1. moves the current .sieve aside to .sieve.bak-<timestamp>
...
from sieve_agent.tools.calendar import make_tool
...
FACTS = [
    ...
    ("user", "The user is a software engineer who prefers concise, direct answers."),
...
def main(reset_spend: bool = False) -> None:
    ...
    print("  Run `sieve_agent dashboard` and start filming.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Reset .sieve to a clean demo state.")
    parser.add_argument("--yes", "-y", action="store_true",
                        help="required confirmation: yes, wipe .sieve (it is backed up first)")
    ...
    if not args.yes:
        ...
        print("REFUSING to run: demo_seed clears .sieve (memory, calendar, chat, traces"
